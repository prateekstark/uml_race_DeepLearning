...
class RaceSolver(object):
    ...
    def do_work(self):
        ...
        if(self.input.shape[0]==0):
        ...
        elif(self.input.shape[0]<10 and self.input.shape[0]>0):
            # print self.input
            self.input = np.concatenate((self.input, X))
            pred_vel = 5
            pred_ang_vel = 0
            ...
            self.input = np.concatenate((self.input, X))
            pred_vel = model1.predict(np.array([self.input]))*5
            pred_ang_vel = model2.predict(np.array([self.input]))*45*math.pi
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            r.sleep()
        exit()
model1 = load_model('uml_race_pred_velocity.h5')
model2 = load_model('uml_race_pred_angular_velocity.h5')
