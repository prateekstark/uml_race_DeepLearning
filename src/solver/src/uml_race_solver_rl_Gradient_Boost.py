...
class DQN(object):
	def __init__(self):
		...
		self.model = self.network('grad_boost_clf.pickle')
		self.epsilon = 0
		self.memory = []
		self.counter_plot = []
		try:
	...
	def set_reward(self, new_state, collision, finish, curr_time):
		...
		if collision:
			...
			self.reward = 1/(abs(new_state[0] - new_state[179]) + 10) + curr_time
		return self.reward

	def network(self, weights=None):
		clf = GradientBoostingClassifier(n_estimators=100, learning_rate=1.0, max_depth=1, random_state=0)
		if os.path.isfile(weights):
			clf = pickle.load(open('grad_boost_clf.pickle', "rb"))
		return clf
	...
	def replay_new(self, memory):
		...
		for state, action, reward, next_state, done in minibatch:
			...
			a = time.time()
			self.model.fit(np.array([state]), target_f)
			b = time.time()
			print(b - a)
	...
	def train_short_memory(self, state, action, reward, next_state, done):
		...
		self.model.fit(state.reshape((1, 180)), target_f)
...
class RaceSolver(object):
	...
	def run(self):
		...
		f.close()
		pickle.dump(model, open('grad_boost_clf.pickle', "wb"))
